# =========================
# 🌍 COUNTRIES DATABASE (100+ mimo Schengen)
# =========================
...
COUNTRIES_DATA = [
    ...
    {"name": "Bangladesh", "iso": "BD", "cz": "banglades", "sk": "bangladéš", "continent": "asie"},
    ...
    {"name": "Dominican Republic", "iso": "DO", "cz": "dominika", "sk": "dominika", "continent": "severni_amerika"},
    ...
    # OSTATNÍ
    {"name": "Australia", "iso": "AU", "cz": "australie", "sk": "australia", "continent": "australie"},
    {"name": "New Zealand", "iso": "NZ", "cz": "novy_zeland", "sk": "novy-zeland", "continent": "australie"},
]
...
def to_api_format(name):
    c = get_country(name)
    return c["iso"] if c else name


def get_mzv_cz_url(name):
    c = get_country(name)
    if not c:
        return None
    return f"https://mzv.gov.cz/jnp/cz/encyklopedie_statu/{c['continent']}/{c['cz']}/cestovani/index.html"


def get_mzv_sk_url(name):
    c = get_country(name)
    if not c:
        return None
    return f"https://www.mzv.sk/web/sk/{c['sk']}"


COUNTRIES = sorted([c["name"] for c in COUNTRIES_DATA])
